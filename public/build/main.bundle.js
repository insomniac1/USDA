...
d3.json("/js/us-data-show.json", function (error, data) {
  ...
  var countiesObj = data.county;
  var stateObj = data.state;

  // console.log(data.county);

  var countyKeys = Object.keys(countiesObj);

  var countyNames = countyKeys.forEach(function (key) {
...
exports.drawBarZoomMap = drawBarZoomMap;
function drawBarZoomMap(data) {
  ...
  var color = d3.scale.ordinal().range(["#76c6cc", "#e8b251", "#44a484"]);

  var svg_bar = d3.select('#zoom-bar-chart').append("svg").attr("width", width_bar + margin.left + margin.right).attr("height", height_bar + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

  if (data) {
    var yearsNames_bar = data.map(function (d) {
      return d.year;
    });
    ...
    y.domain([0, d3.max(data, function (year) {
      return d3.max(year.values, function (d) {
        return d.value;
      });
    })]);

    svg_bar.append("g").attr("class", "x axis").attr("transform", "translate(0," + height_bar + ")").call(xAxis);

    svg_bar.append("defs").append("pattern").attr("id", "img1").attr("patternUnits", "objectBoundingBox").attr("width", "7").attr("height", "68").append("image").attr("xlink:href", "/img/zoom-bar.png").attr("width", "7").attr("height", "68");

    svg_bar.append("defs").append("pattern").attr("id", "img2").attr("patternUnits", "objectBoundingBox").attr("width", "7").attr("height", "68").append("image").attr("xlink:href", "/img/zoom-bar-right.png").attr("width", "7").attr("height", "68");

    svg_bar.select('.y').transition().duration(500).delay(1300).style('opacity', '1');

    var slice = svg_bar.selectAll(".slice").data(data).enter().append("g").attr("class", "g").attr("transform", function (d) {
    ...
    slice.selectAll("rect").data(function (d) {
      ...
      return height_bar - y(0);
    }).on("mouseover", function (d) {
      // d3.select(this).style("fill", d3.rgb(color(d.rate)).darker(2));
    }).attr("rx", 2).attr("ry", 2).on("mouseout", function (d) {
    ...
    var brush = d3.svg.brush().x(x0).on("brush", function () {
      ...
      var selected_brush = [domain[selected_start], domain[selected_end]];

      // console.log(selected_brush);
    });

    svg_bar.append("g").attr("class", "x brush").call(brush).selectAll("rect").attr("y", 0).attr("height", height_bar);

    svg_bar.select('.resize.w').attr("fill", "url(#img1)");

    svg_bar.select('.resize.e').attr("fill", "url(#img2)");
...
exports.drawCurveLine = drawCurveLine;

var _functions = __webpack_require__(/*! ./functions */ "./public/js/functions.js");

function drawCurveLine(dataset_curve) {
    ...
    var xScale_curve = d3.scale.linear().domain([d3.min(dataset_curve, function (d) {
        return d.year;
    }), d3.max(dataset_curve, function (d) {
        return d.year;
    })]).range([0, width]);

    var yScale_curve = d3.scale.linear().domain([0, d3.max(dataset_curve, function (d) {
        return d.yield * 1.1;
    })]).range([height, 0]);

    var xAxis_curve = d3.svg.axis().scale(xScale_curve).orient("bottom").innerTickSize(-height).outerTickSize(0).ticks(5).tickPadding(10).tickFormat(function (d) {
        return d;
    });

    var yAxis_curve = d3.svg.axis().scale(yScale_curve).orient("left").innerTickSize(-width).outerTickSize(100).tickPadding(10).ticks(5).tickFormat(function (d) {
        return d;
    }).tickFormat(function (d) {
        return '';
    });

    var line_curve = d3.svg.line().x(function (d) {
        return xScale_curve(d.year);
    }).y(function (d) {
        return yScale_curve(d.yield);
    }).interpolate("bundle");
    ...
    var area = d3.svg.area().x(function (d) {
        ...
        return yScale_curve(d.yield);
    }).interpolate("bundle").y1(function (d) {
        return height;
    });

    var svg_curve = d3.select("#curve-line").append("svg").attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

    var gradient_svg = $(curve_sp).find('svg')[0];
    (0, _functions.createGradient)(gradient_svg, 'curve-line-gradient', [// Create gradient
    { offset: '0%', 'stop-color': '#44A484 ', 'stop-opacity': '0.45' }, { offset: '75%', 'stop-color': '#FFFFFF', 'stop-opacity': '0.05' }]);

    svg_curve.append("path").attr("d", area(dataset_curve)).attr("class", "streamPath").style('fill', "url(#curve-line-gradient)");

    svg_curve.append("g").attr("class", "x axis").attr("transform", "translate(0," + height + ")").call(xAxis_curve);

    svg_curve.append("g").attr("class", "y axis").call(yAxis_curve);

    svg_curve.append("path").data([dataset_curve]).attr("class", "line").attr("d", line_curve);
}
...
__webpack_require__(/*! ./functions.js */ "./public/js/functions.js");
__webpack_require__(/*! ./autocomplete.js */ "./public/js/autocomplete.js");
__webpack_require__(/*! ./map-counties.js */ "./public/js/map-counties.js");
__webpack_require__(/*! ./bar-zoom-set.js */ "./public/js/bar-zoom-set.js");

__webpack_require__(/*! ./soil-chemistry.js */ "./public/js/soil-chemistry.js");
__webpack_require__(/*! ./temperature-vegetation.js */ "./public/js/temperature-vegetation.js");
__webpack_require__(/*! ./curve-line.js */ "./public/js/curve-line.js");

__webpack_require__(/*! ./script.js */ "./public/js/script.js");

__webpack_require__(/*! core-js */ "./node_modules/core-js/index.js");

/***/ }),

/***/ "./public/js/map-counties.js":
/*!***********************************!*\
  !*** ./public/js/map-counties.js ***!
  \***********************************/
/*! no static exports found */
...
var radius = d3.scale.sqrt().range([2, 40]);

var color_map = d3.scale.threshold().range(["#FBF0DE", "#F7E2BC", "#F2CC89", "#E9BF77", "#DBAB58", "#A5E7D1", "#8EDAC1", "#64C2A2", "#3FA885", "#20906A"]);
...
function initMap() {
  ...
  svg_map = mapDiv.append("svg").attr("class", "map-background").attr("width", width_map).attr("height", height_map).on("click", resetZoom).append("g").attr("transform", "translate(" + mapMargin.left + "," + mapMargin.top + ")");

  // add class for each county
  counties = svg_map.append("g").attr("class", "counties");

  // add class for each state
  states = svg_map.append("g").attr("class", "states");

  // add state contour
  highlight = svg_map.append("g").attr("class", "highlight");

  projection = d3.geo.albersUsa().scale(width_map).translate([width_map_ / 2, height_map_ / 2]);
  ...
  d3.json("/js/us.json", function (error, us) {
    ...
    counties.selectAll(".county").data(topojson.feature(us, us.objects.counties).features).enter().append("path").attr("class", "county").attr("data-county-id", function (d) {
      return d.id;
    }).attr("d", path).on("mouseout", function () {
      destroyTooltip();
      ...
      highlight.append("path").datum(d).attr("class", "highlight-outer").attr("d", path).style("stroke-width", 4.5 / uiState.zoomScale + "px");
      highlight.append("path").datum(d).attr("class", "highlight-inner").attr("d", path).style("stroke-width", 1 / uiState.zoomScale + "px");
    ...
    states.selectAll(".state").data(topojson.feature(us, us.objects.states).features).enter().append("path").attr("class", function (d) {
      return "state state-id-" + d.id;
    }).attr("d", path).on("mouseout", function () {
      highlight.selectAll('*').remove();
      ...
      highlight.append("path").datum(d).attr("class", "highlight-outer").attr("d", path);
      highlight.append("path").datum(d).attr("class", "highlight-inner").attr("d", path);
      changeState(d.id);
    }).on("click", function (d) {
...
x = d3.scale.linear().range([5, width_map_ - 5]).clamp(true);

y = d3.scale.linear().domain([0, 100]).range([lineChartHeight - 5, 5]);

miniSVG = timeseriesDiv.append("svg").attr("width", width_map).attr("height", 140).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");

timeseries = miniSVG.append("g");
timeseries.append("rect").attr("class", "background").attr("width", width_map_).attr("height", lineChartHeight);

yAxis = d3.svg.axis().scale(y).orient("left").ticks(4).tickSize(-width_map_).tickPadding(12).tickFormat(d3.format("s"));

timeseries.append("g").attr("class", "y axis").call(yAxis);

xAxis = d3.svg.axis().scale(x).orient("bottom").tickFormat(function (d) {
  return d;
}).tickSize(-lineChartHeight).ticks(12).tickPadding(12);

timeseries.append("g").attr("class", "x axis").attr("transform", "translate(0," + lineChartHeight + ")").call(xAxis);
...
function resetZoom() {
  ...
  svg_map.transition().duration(500).ease("exp-out").attr('data-zoom', false).attr("transform", "translate(" + mapMargin.left + "," + mapMargin.top + ")");

  setTimeout(function () {
    states.selectAll("*").style("visibility", "visible");
    counties.selectAll(".county").style("visibility", "visible").style("stroke-width", "0.5px");
  }, 500);
...
function zoomed(state, type) {
  ...
  svg_map.transition().duration(500).attr('data-zoom', true).attr("transform", "translate(" + translate + ")scale(" + scale + ")");

  highlight.selectAll('*').remove();

  states.selectAll("*").style("visibility", "hidden");

  counties.selectAll(".county").style("visibility", function (d) {
...
function slided() {
  ...
  handle.selectAll("circle").transition().duration(25).attr("cx", x(value));
...
function updateDataMap() {
  var type = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'yield';
  ...
  d3.json('/js/us-data-' + type + '.json', function (err, json) {
    ...
    timeseries.select("g.x.axis").call(xAxis);

    labels.unit.html(summary.metadata.unit.toLowerCase());

    if (uiState.mode === 'STATE' && !summary.state.hasOwnProperty(uiState.state)) {
      resetZoom();
    } else {
      changeState(uiState.state);
    }

    if (uiState.year && uiState.year <= summary.metadata.yearRange[1] && uiState.year >= summary.metadata.yearRange[0]) {
...
function updateMap() {
  ...
  counties.selectAll(".county").style("fill", function (d) {
    if (summary.county.hasOwnProperty(d.id) && summary.county[d.id].data.hasOwnProperty(uiState.year)) {
...
function updateTimeseries() {
  ...
  var lineData = Object.keys(dat.data).map(function (d) {
    return { x: +d, y: dat.data[d] };
  });

  y.domain(dat.yRange);
  timeseries.select("g.y.axis").call(yAxis);
...
function showTooltip(d) {
  ...
  tooltip.style("visibility", "visible").style("opacity", 1).style("margin-top", "-50px").style("left", x_mouse - tooltipOffset[0] + "px").style("top", y_mouse + "px");
}
...
$("#searchCounty").on("select2:select change.select2", function (e) {
  ...
  setTimeout(function () {
    var itm_county = svg_map.selectAll(".state-id-" + state_id).attr("data-id", function (d) {
      zoomed(d, 'select');
      return d.id;
    });
  }, zoom_delay);
...
var _mapCounties = __webpack_require__(/*! ./map-counties */ "./public/js/map-counties.js");
...
function drawSoilChemistry(soil_chemistry_data) {
  ...
  ** Set up
  ...
  var svg_cicle = d3.select("#chart-circle").append("svg").attr("width", width_circle + margin.left + margin.right).attr("height", height + margin.top + margin.bottom);

  var wrapper = svg_cicle.append("g").attr("class", "chordWrapper").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
  ...
  ** Initialize Axes & Scales
  ...
  var color_cicle = d3.scale.ordinal().range(["#2e8bc2", "#209481", "#B6334F"]).domain(["water", "carbon", "soil_quality"]);

  // Set the new x axis range
  var xScale = d3.scale.log().range([0, width_circle]).domain([d3.min(soil_chemistry_data, function (d) {
    return d.yield * 0.9;
  }), d3.max(soil_chemistry_data, function (d) {
    return d.yield * 1.1;
  })]);

  // Set new x-axis
  var xAxis = d3.svg.axis().orient("bottom").ticks(2).tickFormat(function (d) {
  ...
  // Append the x-axis
  wrapper.append("g").attr("class", "x axis").attr("transform", "translate(" + 0 + "," + height + ")").call(xAxis);

  // Set the new y axis range
  var yScale = d3.scale.linear().range([height, 0]).domain([d3.min(soil_chemistry_data, function (d) {
    return d.value * 0.9;
  }), d3.max(soil_chemistry_data, function (d) {
    return d.value * 1.1;
  })]).nice();

  var yAxis = d3.svg.axis().orient("left").ticks(6) // Set rough # of ticks
  .scale(yScale);

  // Scale for the bubble size
  var rScale = d3.scale.sqrt().range([mobileScreen ? 1 : 2, mobileScreen ? 10 : 16]).domain(d3.extent(soil_chemistry_data, function (d) {
    return d.yield;
  }));
  ...
  ** Circles to capture close mouse event
  ...
  clipWrapper.selectAll(".clip").data(voronoiCells).enter().append("clipPath").attr("class", "clip").attr("id", function (d) {
    if (d) return "clip-" + d.point.code;
  }).append("path").attr("class", "clip-path-circle").attr("d", function (d) {
    if (d) return "M" + d.join(",") + "Z";
  });
  ...
  var circlesOuter = circleClipGroup.selectAll(".circle-wrapper").data(soil_chemistry_data.sort(function (a, b) {
    ...
    return "circle-wrapper circle-type-" + d.type + " " + d.code + " " + (d.type != "water" ? "hidden" : "");
    ...
    return yScale(d.value);
  }).attr("r", maxDistanceFromPoint).on("mouseover", showTooltip).on("mouseout", removeTooltip);
  ...
  ** Scatterplot Circles
  ...
  circleGroup.selectAll("countries").data(soil_chemistry_data.sort(function (a, b) {
    return b.yield > a.yield;
  })) //Sort so the biggest circles are below
  .enter().append("circle").attr("class", function (d, i) {
    return "countries circle-type-" + d.type + " " + d.code + " " + (d.type != "water" ? "hidden" : "");
  }).attr("cx", function (d) {
    return xScale(d.yield);
  }).attr("cy", function (d) {
    return yScale(d.value);
    ...
    return 5;
  }).style("pointer-events", "none").style("opacity", opacityCircles).style("fill", function (d) {
    return color_cicle(d.type);
  });

  var circleGroupShadow = wrapper.append("g").attr("class", "circleWrapperShadow");

  // Place the name circles
  circleGroupShadow.selectAll("countries").data(soil_chemistry_data.sort(function (a, b) {
    return b.yield > a.yield;
  })) // Sort so the biggest circles are below
  .enter().append("circle").attr("class", function (d, i) {
    return "countries-shadow circle-type-" + d.type + " " + d.code + " " + (d.type != "water" ? "hidden" : "");
  }).attr("cx", function (d) {
    return xScale(d.yield);
  }).attr("cy", function (d) {
    return yScale(d.value);
    ...
    return 20;
  }).style("pointer-events", "none").style("opacity", 0).style("fill", function (d) {
    return color_cicle(d.type);
  });
  ...
  ** Hover functions of the circles
  ...
  function showTooltip(d, i) {
    ...
    var elementShadow = d3.selectAll("#chart-circle .countries-shadow." + d.code);
    elementShadow.style("fill", function (d) {
      return color_cicle(d.type);
    });
...
/***/ "./public/js/temperature-vegetation.js":
/*!*********************************************!*\
  !*** ./public/js/temperature-vegetation.js ***!
  \*********************************************/
/*! no static exports found */
...
exports.drawTemperatureVegetation = drawTemperatureVegetation;
...
function drawTemperatureVegetation(data) {
  ...
  // Add scale for x
  var x_area = d3.scale.ordinal().rangeRoundBands([0, width_area], -1.1);

  if (mobile_area) {
    var x_ticks = d3.scale.ordinal().rangeRoundBands([0, width_area], 0);
  } else {
    var x_ticks = d3.scale.ordinal().rangeRoundBands([0, width_area], -0.7);
  }

  // Add scale for y
  var y_area = d3.scale.linear().rangeRound([height_area, 0]);

  // Add x axis
  var xAxis_area = d3.svg.axis()
  // .scale(x_area)
  .scale(x_ticks).orient("bottom").tickPadding(12).tickSize(1).tickFormat(function (d) {
    if (mobile_area) {
      if (d % 3 == 0) return d;
    } else {
      return d;
  ...
  var yAxis_border = d3.svg.axis().scale(y_area).orient("left").innerTickSize(-width_area).outerTickSize(0).tickPadding(10).ticks(5).tickFormat(function (d) {
    return '';
  });

  // Create area border
  var area_border = d3.svg.line().x(function (d, i) {
    return x_area(d.year);
  }).y(function (d) {
    return y_area(d.value);
  });

  // Add colors according to the design
  var color_area = d3.scale.ordinal().range(["#2D9280", "#5BABB1", "#E5B156"]).domain(["yield", "temperature", "vegetation"]);

  // Add SVG for the area chart to the relevant container (/views/pages/landing.js)
  var svg_area = d3.select(area_ContainerID).append("svg").attr("width", width_area + margin.left + margin.right).attr("height", height_area + margin.top + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")");
  ...
  ** Output for 'x_area.domain()': (..) [2000, 2001, 2002, 2003, ..]
  ...
  svg_area.append("g").attr("class", "y axis").call(yAxis_border);

  // Create g element for X axis labels (in our case: 2000, 2001, 2002, ..)
  svg_area.append("g").attr("class", "x axis").attr("transform", "translate(0," + height_area + ")").call(xAxis_area);

  //Add relevant classes to each series data
  var selection = svg_area.selectAll(".series").data(seriesArr).enter().append("g").attr("class", function (d, i) {
  ...
  ** Fill each area with the relevant color
  ...
  (0, _functions.createGradient)(gradient_svg, 'temperature-gradient-0', [// Create gradient
  { offset: '0%', 'stop-color': '#44A484 ', 'stop-opacity': '0.45' }, { offset: '75%', 'stop-color': '#44A484', 'stop-opacity': '0.05' }]);
  (0, _functions.createGradient)(gradient_svg, 'temperature-gradient-1', [// Create gradient
  { offset: '0%', 'stop-color': '#76C6CC', 'stop-opacity': '0.45' }, { offset: '75%', 'stop-color': '#76C6CC', 'stop-opacity': '0.05' }]);
  (0, _functions.createGradient)(gradient_svg, 'temperature-gradient-2', [// Create gradient
  { offset: '0%', 'stop-color': '#E8B251 ', 'stop-opacity': '0.45' }, { offset: '75%', 'stop-color': '#E8B251', 'stop-opacity': '0.05' }]);
  // 2. Fill chart area
  selection.append("path").attr("d", function (d) {
    return area(d.values);
  }).attr("class", "streamPath").style('fill', function (d, i) {
  ...
  */
  ...
  // Visible points
  var points = svg_area.selectAll(".seriesPoints").data(seriesArr).enter().append("g").attr("class", "seriesPoints");

  points.selectAll(".point").data(function (d) {
    return d.values;
  }).enter().append("circle").attr("class", function (d, i) {
    return "point point-itm-" + d.name + '-' + i;
  }).attr("cx", function (d) {
    return x_area(d.year) + x_area.rangeBand() / 2;
  }).attr("cy", function (d) {
    return y_area(d.y);
  }).attr("r", "5px").style("opacity", 0).style("fill", function (d) {
    return color_area(d.name);
  });

  // Hidden points
  var points_hover = svg_area.selectAll(".seriesPointsHover").data(seriesArr).enter().append("g").attr("class", function (d, i) {
    return "seriesPointsHover seriesPointsHover-" + i;
  });

  if (data[0].years.length != 1 && data[0].years[0].state != "") {
    points_hover.selectAll(".point-hover").data(function (d) {
      return d.values;
    }).enter().append("circle").attr("class", "point-hover").attr("cx", function (d) {
      return x_area(d.year) + x_area.rangeBand() / 2;
    }).attr("cy", function (d) {
      return y_area(d.y);
    }).attr("r", "25px").style("opacity", 0).style("fill", function (d) {
      return color_area(d.name);
    }).on("mouseover", function (d, i) {
      showPopover.call(this, d, i);
    }).on("mouseout", function (d, i) {
      removePopovers.call(this, d, i);
    });
  }
  ...
  ** Line styles are described in _section.scss:294-307
  ...
  seriesArr.forEach(function (d, i) {
    svg_area.select(".series-" + i).append("path").data([seriesArr[i].values]).attr("class", "border-line line" + i).attr("d", area_border).attr("transform", "translate(" + x_area.rangeBand() / 2 + ", 0)");
  });
