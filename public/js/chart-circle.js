function drawFinalScatterplot() {
  ...
  function removeTooltip (d, i) {
    ...
    var element = d3.selectAll("#chart-final .countries."+d.CountryCode);
    ...
    d3.selectAll("#chart-final .guide")
...
function drawLegend(svgName, svgLegendName, opacityCircles, color, rScale) {
  //Legend      
  var legendMargin = {left: 5, top: 10, right: 5, bottom: 10},
    legendWidth = 145,
    legendHeight = 270;
    
  var svgLegend = d3.select("#" + svgLegendName).append("svg")
  ...
  var legend = legendWrapper.selectAll('.legendSquare')   
        ...
        .on("mouseover", selectLegend(0.02, svgName))
        .on("mouseout", selectLegend(opacityCircles, svgName));
  ...
  function selectLegend(opacity, svgName) {
    return function(d, i) {
      var chosen = color.domain()[i];
        
      d3.selectAll("#" + svgName + " .countries")
